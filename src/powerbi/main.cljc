(ns powerbi.main
  "Kotodama WASM entrypoint for the Powerbi clean-room actor (L5) — Clojure port.
  ...
  facts (`powerbi.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "powerbi")
(def tier "L5")
...
(def entity-specs
  [{:entity "Dataset"   :plural "datasets"   :id-prefix "powerbi_dat"
    :fields [:name :description :contentProviderType :targetStorageMode :isRefreshable :addRowsAPIEnabled] :required [:name :description]
    :coerce {:isRefreshable :bool :addRowsAPIEnabled :bool} :refs {}}
   {:entity "Report"    :plural "reports"    :id-prefix "powerbi_rep"
    :fields [:name :description :reportType :datasetId :isOwnedByMe] :required [:name :description]
    :coerce {:isOwnedByMe :bool} :refs {:datasetId "Dataset"}}
   {:entity "Dashboard" :plural "dashboards" :id-prefix "powerbi_das"
    :fields [:displayName :isReadOnly] :required [:displayName :isReadOnly]
    :coerce {:isReadOnly :bool} :refs {}}
   {:entity "Group"     :plural "groups"     :id-prefix "powerbi_gro"
    :fields [:name :isReadOnly :isOnDedicatedCapacity :defaultDatasetStorageFormat] :required [:name :isReadOnly]
    :coerce {:isReadOnly :bool :isOnDedicatedCapacity :bool} :refs {}}
   {:entity "Tile"      :plural "tiles"      :id-prefix "powerbi_til"
    :fields [:title :reportId :datasetId :rowSpan :colSpan] :required [:title :rowSpan]
    :coerce {:rowSpan :int :colSpan :int} :refs {:reportId "Report" :datasetId "Dataset"}}
   {:entity "Gateway"   :plural "gateways"   :id-prefix "powerbi_gat"
    :fields [:name :type :gatewayStatus] :required [:name :type]
    :coerce {} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"powerbi.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "powerbi-compat" :tier tier :entities entities} 200])
...
(defn start! [] :powerbi-compat/ready)
